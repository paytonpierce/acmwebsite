...
function OfficerPage() {
    ...
    const { loading, error, data = [] } = useFetch('https://6w4o0fw74l.execute-api.us-east-2.amazonaws.com/officersget', {}, []);
    const years: number[] = []
    ...
    data.map((value: officerType) => {
        ...
        officers.get(value.year)?.push(<Officer firstName={value.firstName} lastName={value.lastName} position={value.position} image={value.image}/>)
    })
        ...
        <div className='officer-group'>
            ...
            <div>
                <select className="officer-subtitle" id="yearSelect">
                    <option value={2022} key={`${2022}-${2022+1}`} onClick={() => selection(2022)}>{`${2022}-${2022+1}`}</option>
                    <option value={2021} key={`${2021}-${2021+1}`} onClick={() => selection(2021)}>{`${2021}-${2021+1}`}</option>
                </select>
            </div>
            {selectedYear}
        </div>
